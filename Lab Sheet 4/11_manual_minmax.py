...
def manual_minmax(series):
    minv = series.min(); maxv = series.max()
    return (series - minv) / (maxv - minv)

if __name__ == '__main__':
    df['x2_manual_norm'] = manual_minmax(df['x2'])
    print(df[['x2','x2_manual_norm']].head())
    df[['x2','x2_manual_norm']].to_csv('outputs/11_manual_norm.csv', index=False)
    print('Saved outputs/11_manual_norm.csv')
