...
if __name__ == '__main__':
    df['target'] = df['x1']*0.6 + df['x3']*0.3 + np.random.normal(size=len(df))
    X = df[['x1','x2','x3']]
    y = df['target']
    ...
    scores.to_csv('outputs/20_selectkbest_scores.csv', header=['score'])
    print('Saved outputs/20_selectkbest_scores.csv')
