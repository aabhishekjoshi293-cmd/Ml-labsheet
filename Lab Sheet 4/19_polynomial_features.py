...
if __name__ == '__main__':
    ...
    num = df[['x1','x2']]
    pf = poly.fit_transform(num)
    cols = poly.get_feature_names_out(['x1','x2'])
    pd.DataFrame(pf, columns=cols).to_csv('outputs/19_polynomial_features.csv', index=False)
    print('Saved outputs/19_polynomial_features.csv with columns: ' + ','.join(cols))
