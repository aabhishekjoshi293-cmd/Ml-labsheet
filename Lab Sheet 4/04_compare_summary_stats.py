...
def remove_outliers_iqr_df(df, cols):
    df2 = df.copy()
    for c in cols:
        q1 = df2[c].quantile(0.25); q3 = df2[c].quantile(0.75); iqr=q3-q1
        lower=q1-1.5*iqr; upper=q3+1.5*iqr
        df2 = df2[(df2[c] >= lower) & (df2[c] <= upper)]
    return df2
if __name__ == '__main__':
    cols = ['x1','x2','x3']
    before = df[cols].agg(['mean','median','std']).T
    after_df = remove_outliers_iqr_df(df, cols)
    after = after_df[cols].agg(['mean','median','std']).T
    print('Before:\n', before)
    print('\nAfter:\n', after)
